 ...
 *   2. Run migrations:  pnpm db:migrate --name init
...
async function main() {
  ...
  const mainData = XLSX.utils.sheet_to_json(wb1.Sheets[wb1.SheetNames[0]]);

  const wb2 = XLSX.readFile(csPath);
  const caseData = XLSX.utils.sheet_to_json(wb2.Sheets[wb2.SheetNames[0]]);
  ...
  const nanomaterialMap = {}; // sampleId → id
  ...
  for (const row of mainData) {
    ...
    nanomaterialMap[sampleId] = created.id;
    console.log(`✔ Nanomaterial [${sampleId}]: ${created.name}`);
  }

  // ── 4. Seed case studies ──────────────────────────────────────────────────
  // Simple keyword matcher to link case-study rows to nanomaterials by sampleId
  function findNanomaterialId(csName) {
    if (!csName) return null;
    const normalised = csName.toLowerCase().replace(/\s+/g, " ").trim();
    ...
    for (const [kw, sid] of keywords) {
      if (normalised.includes(kw)) {
        return nanomaterialMap[sid] ?? null;
      }
    }
    return null;
  }
